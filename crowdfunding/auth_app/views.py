    ...
    if 'user_id' not in req.session:
        ...
        if req.method == "POST":
            ...
            if form.is_valid():
                ...
                if user is not None and user.is_delete != True:
                    if user.is_active:
...
def resetPassword(req):
    if 'user_id' not in req.session:
        if req.method == "POST":
            if req.POST['email']:
                user = RegisterUser.objects.filter(user_email=req.POST['email'],is_delete=False).first()
                if user:
...
def sendEmailPassword(req, email):
    if 'user_id' not in req.session:
        if req.method == "POST":
            ...
            if form.is_valid():
                user = RegisterUser.objects.filter(user_email=email,is_delete=False).first()
                if user:
                    user.user_password = form.cleaned_data['user_password']
                    user.save()
                    return redirect('login')
            ...
            return render(req, 'auth/setpassword.html',
                          context={"title": "login", "email": email, "form": PasswordForm()})
    else:
        return redirect("login")
